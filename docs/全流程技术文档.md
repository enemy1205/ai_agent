...
│                                                                   │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  pipeline.py (语音监听与交互程序)                          │  │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐ │  │
│  │  │ 麦克风采集 │→│ VAD检测   │→│ 声纹认证  │→│ ASR识别   │ │  │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘ │  │
│  │         ↓                                          ↓        │  │
│  │  ┌──────────┐                              ┌──────────┐ │  │
│  │  │ TTS播放   │←─────────────────────────────│ LLM处理  │ │  │
...
│                              ↕ HTTP                              │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  mqtt_manager.py (MQTT-ROS桥接 + 任务队列)                  │  │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐ │  │
│  │  │MQTT订阅  │→│ 任务队列  │→│ ROS发布   │→│ 机器人执行 │ │  │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘ │  │
│  └──────────────────────────────────────────────────────────┘  │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
...
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐               │  │
│  │  │ Flask API │→│ LangChain │→│ LLM推理   │               │  │
│  │  │  接口     │  │  Agent   │  │  引擎     │               │  │
│  │  └──────────┘  └──────────┘  └──────────┘               │  │
│  │         ↓                                                  │  │
│  │  ┌──────────────────────────────────┐                    │  │
│  │  │   robot_tools.py (机器人工具库)    │                    │  │
│  │  │   ┌────────┐ ┌────────┐ ┌──────┐ │                    │  │
│  │  │   │导航工具 │ │机械臂  │ │夹爪  │ │                    │  │
│  │  │   └────────┘ └────────┘ └──────┘ │                    │  │
│  │  └──────────────────────────────────┘                    │  │
...
用户说话
   ↓
[1] 语音采集 (pipeline.py)
   ├─ PyAudio实时采集音频 (16kHz, Float32)
   ├─ Silero VAD检测语音活动
   └─ 捕获语音片段
   ↓
[2] 语音识别 (voice_services.py)
   ├─ 接收Base64编码的WAV音频
   ├─ 调用腾讯云ASR API
   └─ 返回识别文本
   ↓
[3] 检查特殊指令 (pipeline.py)
   ├─ 是否包含"注册新用户"?
   │  └─ 是 → 进入注册模式
   └─ 否 → 继续正常流程
   ↓
[4] 声纹认证 (可选, voice_services.py)
   ├─ 提取声纹特征 (WeSpeaker)
   ├─ 与数据库比对
   └─ 返回认证结果
   ↓
[5] AI对话处理 (http_agent_server.py)
   ├─ 接收识别文本
   ├─ 构建对话历史
   ├─ LLM生成回复
   └─ 可能触发工具调用
   ↓
[6] 工具执行 (robot_tools.py, 如果需要)
   ├─ 识别需要调用的工具
   ├─ 通过MQTT发送控制指令
   └─ 返回执行结果
   ↓
[7] 语音合成 (voice_services.py)
   ├─ 接收回复文本
   ├─ 调用腾讯云TTS API
   └─ 返回Base64编码的音频
   ↓
...
[2] LLM分析意图
   ├─ 识别地点: office
   ├─ 识别动作: 拿取
   └─ 选择工具: get_water_bottle()
   ↓
[3] 工具调用 (robot_tools.py)
   ├─ get_water_bottle() 函数
   ├─ 连接MQTT Broker
   └─ 按顺序发送指令:
       ├─ 步骤0: 发送夹爪松开指令
...
[1] ASR识别关键词
   └─ 检测到 "注册新用户"
   ↓
[2] 进入注册模式 (pipeline.py)
   ├─ 设置 is_register_mode = True
   ├─ 生成用户ID: user_<timestamp>
   └─ TTS提示: "请说一段话用于注册，您的用户ID是..."
   ↓
[3] 用户说话 (注册语音)
   ├─ VAD捕获语音
   └─ ASR识别(仅用于确认)
   ↓
[4] 声纹注册 (voice_services.py)
   ├─ 提取声纹特征 (WeSpeaker)
   ├─ 保存到数据库 (speaker_db/)
   └─ 返回注册结果
   ↓
