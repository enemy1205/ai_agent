...
class NavigationManager:
    def __init__(self):
        ...
        rospy.loginfo("任务队列执行器已启动")
        rospy.loginfo(f"任务完成后等待时间: {self.task_completion_delay}秒") 
    ...
    def update_nav_status(self, data):
        ...
        with self.lock: 
            ...
            if current_status != self.last_nav_status:
                ...
                rospy.loginfo(f"当前导航状态: {self.nav_status} ({status_desc.get(current_status, '未知')})")

    def publish_navigation(self, x, y, z, orientation=None):
        """直接发布导航目标"""
        rospy.loginfo(f"发布导航目标: ({x}, {y}, {z})")
        if orientation:
            rospy.loginfo(f"目标朝向: {orientation}")
        ...
        if orientation and isinstance(orientation, dict):
            ...
            rospy.loginfo(f"使用自定义朝向: ({q.x}, {q.y}, {q.z}, {q.w})")
        else:
            # 使用默认朝向(0,0,0,1)
            q = rpy2elements(0, 0, 0)
            goal.pose.orientation = q
            rospy.loginfo("使用默认朝向: (0, 0, 0, 1)")

        # 发布目标
        self.pub_nav_goal.publish(goal)
        rospy.loginfo(f"已发布导航目标到 /move_base_simple/goal")
    ...
    def publish_arm_command(self, command):
        ...
        if command == 0:
            ...
            rospy.logwarn(f"未知命令: {command}，不执行任何操作")
            return

        # 发布机械臂控制指令
        self.pub_arm_drive.publish(arm_cmd)
        rospy.loginfo(f"已发布机械臂指令到 /arm_drive: {arm_cmd}")
    ...
    def publish_arm_coordinate(self, x, y, z, rx, ry, rz):
        ...
        # 发布机械臂控制指令
        self.pub_arm_drive.publish(arm_cmd)
        rospy.loginfo(f"已发布机械臂坐标指令到 /arm_drive: {arm_cmd}")

    def publish_gripper_command(self, command):
        """直接发布夹爪命令"""
        rospy.loginfo(f"发布夹爪命令: {command}")
        ...
        rospy.loginfo(f"夹爪命令: {command_desc}（指令值: {command}）")
        
        # 直接发布
        self.pub_cmd_end_effector.publish(Int32(command))
        rospy.loginfo(f"已将夹爪命令 {command} 发送到 /cmdeffector 话题")

    def add_task(self, task_type, payload):
        """添加任务到队列"""
        with self.queue_lock:
            if self.task_paused:
                rospy.logwarn(f"任务队列已暂停,拒绝添加新任务: {task_type.value}")
                return False
            
            task = Task(task_type, payload)
            self.task_queue.append(task)
            rospy.loginfo(f"任务已加入队列: {task}, 当前队列长度: {len(self.task_queue)}")
            return True
    
    def _task_executor(self):
        """任务执行器主循环"""
        rospy.loginfo("任务执行器线程已启动")
                ...
                with self.queue_lock:
                    ...
                    if self.current_task is not None:
                        if self._is_task_completed(self.current_task):
                            rospy.loginfo(f"任务完成: {self.current_task}")
                            # 记录任务完成时间,开始等待
                            self.task_completion_time = time.time()
                            self.current_task = None
                            rospy.loginfo(f"等待{self.task_completion_delay}秒后执行下一个任务...")
                        else:
                            # 任务未完成,继续等待
                            rate.sleep()
                            continue
    ...
    def _is_task_completed(self, task):
        ...
        if task.task_type == TaskType.NAVIGATION:
            ...
            if status == 1:
                rospy.logerr("导航异常! 终止当前任务并暂停任务队列")
                self._pause_task_queue()
                return True
            elif status == 4:
                rospy.logwarn("检测到手柄控制模式! 终止当前任务并暂停任务队列")
                self._pause_task_queue()
                return True
            elif status == 2:
                rospy.loginfo("导航任务已完成")
                return True
            elif status == 3:
                # 正在导航中,继续等待
                return False
            else:
                rospy.logwarn(f"未知导航状态: {status}, 继续等待")
                return False
    ...
    def _pause_task_queue(self):
        ...
        with self.queue_lock:
            ...
            # 清空当前任务
            self.current_task = None
            rospy.logwarn(f"任务队列已暂停! 队列中剩余 {len(self.task_queue)} 个任务被取消")
            # 清空队列
    ...
    def set_task_completion_delay(self, delay):
        ...
        """
        if delay < 0:
            rospy.logwarn(f"等待时间不能为负数,保持当前值: {self.task_completion_delay}秒")
            return
        
        old_delay = self.task_completion_delay
        self.task_completion_delay = delay
        rospy.loginfo(f"任务完成等待时间已更新: {old_delay}秒 → {delay}秒")
...
def handle_navigation(payload):
    ...
    if isinstance(payload, dict):
        ...
        rospy.logwarn(f"导航指令格式不正确: {payload}")


def handle_arm_command(payload):
    """处理机械臂控制指令 - 添加到任务队列"""
    rospy.loginfo("收到机械臂控制指令")
...
def handle_arm_coordinate_command(payload):
    ...
    if isinstance(payload, dict):
        ...
        rospy.loginfo(f"机械臂坐标指令: 位置({x}, {y}, {z}), 姿态({rx}, {ry}, {rz})")
        
        # 添加到任务队列
        if nav_manager:
            nav_manager.add_task(TaskType.ARM_COORDINATE, payload)
    else:
        rospy.logwarn("带坐标机械臂指令格式不正确，需为字典类型")

def handle_gripper_command(payload):
    """处理机械爪控制指令 - 添加到任务队列"""
    rospy.loginfo("收到机械爪控制指令")
